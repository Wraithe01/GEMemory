...
static const char* ImguiGetClipText(void*) 
{
	return GetClipboardText();
}

static void ImguiSetClipText(void*, const char* text)
{
    ...
    if ((io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) == 0)
	{
		...
		if (cursor != g_cursorCurrent || io.MouseDrawCursor)
		{
			...
			else
			{
				...
				{
					SetMouseCursor((cursor > -1 && cursor < ImGuiMouseCursor_COUNT) ? g_cursorTable[cursor] : MOUSE_CURSOR_DEFAULT);
				}
			}
		}
	}
...
void ImguiPushFont(bool bold)
{
    ImGui::PushFont(bold ? g_fontBold : g_font);
}
